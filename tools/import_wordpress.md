...
(下記オプション一覧参照)。`content/_index.md` や `content/_footer.md` など、このツールが生成しない
ファイルは `--refresh` を付けても削除されません。

実行後、コンソールに種別ごとの件数 (全件数・新規・既存)、新たに追加された記事の一覧、
`content/errors` へ振り分けられた記事の一覧、ダウンロードに失敗した画像・添付ファイルの一覧が
表示されます。
...
| `--output PATH` | 出力先ディレクトリ (既定: `content`) |
...
| `--refresh` | `content` 下の生成済みファイル (`content/blog` 等) を事前に全て削除してから再インポートする。指定しない場合 (既定) は削除を行わず、未インポートの記事・ページだけを追加する |
...
- WordPress の投稿 (`post`) は、カテゴリによって次のように振り分けます。
  ...
  - 「しごと情報」カテゴリのみ → `content/job/{WordPress の投稿ID}.md`
  - 「ブログ」と「しごと情報」の**両方**のカテゴリを持つ → `content/errors/{WordPress の投稿ID}.md`
    (自動振り分けできないため、手動での確認・分類が必要です。front matter に
    `wordpress_import_error` としてその旨を記載しています)
  - 上記以外のカテゴリ (または未分類) の記事 → 「トップページ以外のページ」として
    `content/pages/{WordPress の投稿ID}.md`
- WordPress の固定ページ (`page`) は、タイトルが「お問い合わせ」のものだけを
  `content/contact.md` (単一ファイル) として扱います。それ以外の固定ページは
  `content/pages/{WordPress の投稿ID}.md` に配置します。
...
- **blog / job / errors** の記事に画像・添付ファイルがある場合、その記事は Hugo の
  **Page Bundle** (`content/blog/{WordPress の投稿 ID}/index.md`) として出力され、
  画像・添付ファイルは `{WordPress の投稿 ID}_{連番3桁}.{拡張子}` という名前で
  `index.md` と同じディレクトリに配置します (例: `content/blog/4779/index.md` と
  `content/blog/4779/4779_001.png`)。本文中の参照は同じディレクトリ内のファイルを指す
  相対パスに書き換えます (例: `![](4779_001.png)`)。
  画像・添付ファイルがない記事は、従来通り単一の Markdown ファイル
  (`content/blog/{WordPress の投稿 ID}.md`) として出力されます。どちらの形式でも
  Hugo の公開 URL (例: `/blog/4779/`) は変わりません。
- **それ以外 (pages / contact)** の画像・添付ファイルは `content/images/` の下に、
...
- `content/pages/4731.md` にあった WordPress の「最新の投稿」ブロック (wp:latest-posts) は
