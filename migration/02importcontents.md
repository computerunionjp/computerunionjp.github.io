...
「ブログ」および「しごと情報」の両方のカテゴリの記事がある場合は `content/errors` の下に配置してください。
...
<a href="https://example.com" target="_blank" rel="noopener noreferrer">
  リンクテキスト<span class="material-symbols-outlined">open_in_new</span>
</a>
```
...
`content/pages/4731.md` の「しごと情報」と「ブログ」のセクションにはそれぞれの最近の5件の記事のリンクを埋め込みます。
Hugo　の仕様として可能な場合、埋め込みのためのプレースホルダーを記載してください。

### 3.5. 追加の仕様(5)

`tools/import_wordpress.md` に引数 `--refresh` を追加してください。
`content` の下のファイルを削除してから再インポートするための引数です。
引数 `--refresh` が指定されていない場合は `content` の下のファイルを削除せず、
インポートされていない投稿 ID の記事だけを追加します。

`tools/import_wordpress.md` に引数 `--start` を追加してください。
WordPress の投稿 ID が 3500 より古い記事を移行対象外とする仕様を変更し、
引数 `--start` に指定された投稿 ID より古い記事を移行対象外とします。

### 3.6. 追加の仕様(6)

画像を含むページは Page Bundle にして下さい。例えば
